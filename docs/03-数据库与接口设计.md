...
用途：保存站内业务通知。P5-A 已落地该表，用户端用于“我的通知”和未读提醒，管理端用于通知记录查询。
...
| `GET` | `/notifications/mine` | 我的通知列表 | 需要 |
| `GET` | `/notifications/unread-count` | 我的未读通知数 | 需要 |
| `PUT` | `/notifications/{id}/read` | 标记单条已读 | 需要 |
| `PUT` | `/notifications/read-all` | 全部标记已读 | 需要 |
...
| `PUT` | `/activities/{id}/status` | 发布、下架、结束、取消活动 | `manager:voluntary:activity:edit` |
...
| `GET` | `/statistics/activities` | 活动维度统计 | `manager:voluntary:statistics:view` |
| `GET` | `/statistics/volunteers` | 志愿者服务排行 | `manager:voluntary:statistics:view` |
| `GET` | `/statistics/organizations` | 组织维度统计 | `manager:voluntary:statistics:view` |
| `GET` | `/statistics/trend` | 服务趋势统计 | `manager:voluntary:statistics:view` |
| `GET` | `/notifications/list` | 通知记录列表 | `manager:voluntary:notification:list` |
| `GET` | `/notifications/{id}` | 通知详情 | `manager:voluntary:notification:query` |
...
| 通知记录 | `voluntary/notification/index` | `manager:voluntary:notification:list` |

按钮权限建议：

1. `manager:voluntary:volunteer:audit`
2. `manager:voluntary:volunteer:export`
3. `manager:voluntary:activity:add`
4. `manager:voluntary:activity:edit`
5. `manager:voluntary:signup:review`
6. `manager:voluntary:signup:export`
7. `manager:voluntary:checkin:qr`
8. `manager:voluntary:checkin:export`
9. `manager:voluntary:serviceRecord:export`
10. `manager:voluntary:statistics:export`
11. `manager:voluntary:notification:query`

## 11. 设计结论

数据库设计应围绕 `vol_volunteer_profile`、`vol_activity`、`vol_activity_signup`、`vol_checkin_record`、`vol_service_record` 和 `vol_notification` 展开，审核记录作为辅助留痕。接口设计应分为用户端和管理端两套路径，管理端全部纳入权限控制。
