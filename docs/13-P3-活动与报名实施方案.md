...
| P3-A | 数据库骨架 | `vol_activity`、`vol_activity_signup`、字典、菜单 SQL | SQL 可导入 |
| P3-B | 后端实体与 Mapper | domain、mapper、XML | Maven 编译通过 |
| P3-C | 管理端活动接口 | 活动列表、详情、新增、编辑、状态变更 | 接口冒烟通过 |
...
本方案完成后，下一步进入 P3-A 数据库骨架：

1. 在 `sql/ruoyi_voluntary.sql` 和 `sql/ruoyi_voluntary_minimal.sql` 中新增 `vol_activity`。
2. 新增 `vol_activity_signup`。
3. 新增 `vol_activity_status` 和 `vol_signup_status` 字典。
4. 新增活动管理和报名管理菜单及按钮权限。
5. 重新导入两个 SQL 脚本验证可用。
