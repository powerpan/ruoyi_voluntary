...
| P3-D | 用户端活动接口 | 活动列表、详情、报名、我的报名 | 已完成，接口冒烟通过 |
| P3-E | 管理端报名接口 | 报名列表、详情、筛选 | 已完成，接口冒烟通过 |
| P3-F | 管理端页面 | 活动管理、报名管理页面和菜单 | 已完成，构建和浏览器验证通过 |
| P3-G | 用户端页面 | 活动列表、详情、我的报名页面 | 已完成，用户端构建和浏览器验证通过 |
| P3-H | 阶段验收 | P3 验收记录 | 已完成，活动发布到报名筛选闭环通过 |
...
P3-D 用户端活动接口已完成，详细记录见 `docs/17-P3-用户端活动接口记录.md`。
P3-E 管理端报名接口已完成，详细记录见 `docs/18-P3-管理端报名接口记录.md`。
P3-F 管理端页面已完成，详细记录见 `docs/19-P3-管理端页面记录.md`。
P3-G 用户端页面已完成，详细记录见 `docs/20-P3-用户端页面记录.md`。
P3-H 阶段验收已完成，详细记录见 `docs/21-P3-阶段验收记录.md`。

下一步进入 P4 二维码签到签退与服务记录：
...
1. 新增 `VolAppActivityController` 和 `VolAppSignupController`。
2. 补齐用户端已发布活动列表、详情、报名、我的报名和取消报名服务方法。
3. 复用 P2 志愿者审核状态，未审核通过的志愿者不能报名。
4. 取消报名后可重新报名，并通过更新原记录避免违反唯一约束。
5. `mvn -pl ruoyi-admin -am compile -DskipTests` 执行通过。
6. `mvn -pl ruoyi-admin -am package -DskipTests` 执行通过。
7. 临时后端运行级验证通过：匿名活动列表、匿名活动详情、志愿者注册、资料维护、管理员审核、活动报名、我的报名、取消报名、取消后重新报名接口均返回预期状态。

## 16. P3-E 验证记录

P3-E 已完成以下验证：

1. 新增 `VolManagerSignupController`。
2. 补齐管理端报名列表、详情和筛选服务方法。
3. 筛选状态只允许通过、拒绝或候补。
4. 只有待筛选和候补状态允许筛选。
5. 通过报名时会校验招募人数上限并同步活动 `approved_count`。
6. `mvn -pl ruoyi-admin -am compile -DskipTests` 执行通过。
7. `mvn -pl ruoyi-admin -am package -DskipTests` 执行通过。
8. 临时后端运行级验证通过：管理端报名列表、报名详情、候补、通过、拒绝、招募人数上限拦截、已拒绝报名再次筛选拦截均返回预期状态。
...
1. 新增管理端活动和报名 API 封装。
2. 新增活动管理页面，支持列表、筛选、新增、编辑、详情和状态操作。
3. 新增报名管理页面，支持列表、筛选、详情、通过、拒绝和候补操作。
4. `cd ruoyi-ui && npm run build:prod` 执行通过，仅保留已有资源体积 warning。
5. 浏览器验证到活动管理页面可渲染测试活动，活动详情弹窗可打开。
6. 浏览器验证到报名管理页面可渲染测试报名，报名通过弹窗可打开，提交后列表状态刷新。
7. 桌面端 1280 宽度下页面无空白、无框架错误、无页面级横向溢出；移动端 390 宽度下页面可渲染。
...
1. 新增用户端活动和报名 API 封装。
2. 活动列表页面接入真实接口，支持活动搜索、类型筛选、只看报名中、分页和详情入口。
3. 活动详情页面接入真实接口，支持游客浏览、未登录引导登录、未审核通过提示、审核通过志愿者提交报名。
4. 我的报名页面接入真实接口，支持报名记录展示、按活动名称和状态筛选、查看活动和取消报名。
5. 首页阶段文案已从 P2 更新到 P3 活动与报名。
6. `cd voluntary-app && npm run build:prod` 执行通过，仅保留已有资源体积 warning。
7. 浏览器验证到 `/activities` 可展示 `P3-G用户端演示活动`，活动详情和报名弹窗可打开。
8. 浏览器验证到 `p3g_demo_volunteer / 123456` 登录后可提交报名，`/signups` 可展示待筛选报名记录。
9. 移动端 390 宽度下我的报名页面可渲染，控制台无相关 error/warn。

## 19. P3-H 验收记录

P3-H 已完成以下验证：

1. 管理端接口创建 `P3-H闭环验收活动`，初始状态为草稿。
2. 管理端接口发布活动，活动状态变为已发布并写入发布时间。
3. 用户端匿名活动列表和详情可查询到该活动。
4. 审核通过志愿者 `p3g_demo_volunteer` 可提交报名，报名初始状态为待筛选。
5. 管理端报名列表可查询到报名人员 `P3G演示志愿者`。
6. 管理员 `admin` 将报名筛选为通过，并写入处理意见。
7. 活动 `approvedCount` 更新为 `1`，与已通过报名数一致。
8. 用户端“我的报名”可展示通过状态和处理意见。
9. 管理端报名管理页面可展示同一条报名记录，状态为“通过”。
